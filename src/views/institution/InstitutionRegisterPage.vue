<template>
  <ion-page>
        ...
        <form @submit.prevent="onSubmitUi">
          <ion-card>
            ...
            <ion-card-content>
              <ion-item>
                ...
                <ion-input v-model="projectForm.name" required
                  placeholder="Ex: Horta Comunitária do Bairro"></ion-input>
              </ion-item>
              <ion-item>
              ...
              <ion-item>
                ...
                <ion-textarea v-model="projectForm.description" placeholder="Descreva a missão e as atividades."
                  auto-grow></ion-textarea>
              </ion-item>

              <ion-item lines="none">
                <ion-label position="stacked">Categorias do Projeto</ion-label>
              </ion-item>
              ...
              <div class="chips-container" v-if="!categoriesLoading && !categoriesError">
                <ion-chip v-for="category in categoryOptions" :key="category"
                  :color="projectForm.categories.includes(category) ? 'primary' : 'default'"
          ...
          <ion-card>
            <ion-card-header>
              ...
              <ion-card-subtitle>Serviços solidários são oportunidades que necessitam de voluntários. Descreva aqui um
                serviço solidário e chame todos à ação!</ion-card-subtitle>
            </ion-card-header>
            <ion-card-content>
              <ion-item>
                <ion-label position="stacked">Título do Serviço</ion-label>
                <ion-input v-model="serviceForm.title" required
                  placeholder="Ex: Ajudante para plantio de mudas"></ion-input>
              </ion-item>
              <ion-item>
                <ion-label position="stacked">Descrição do Serviço</ion-label>
                <ion-textarea v-model="serviceForm.description" required auto-grow
                  placeholder="Descreva as atividades, requisitos e objetivos."></ion-textarea>
              </ion-item>
    ...
    <ion-footer>
      <ion-toolbar>
        <ion-button class="ma-10" expand="block" @click="onSubmitUi" :disabled="submitting || !isFormValid">
...
const projectForm = reactive({ name: '', address: '', city: '', state: '', description: '', categories: [] as string[] });
const serviceForm = reactive({ title: '', description: '', imageUrl: '' });
...
const isFormValid = computed(() => {
  const isProjectValid = projectForm.name && projectForm.address && projectForm.city && projectForm.state && projectForm.categories.length > 0;
  const isServiceValid = serviceForm.title && serviceForm.description;
...
const toggleCategory = (category: string) => {
  const index = projectForm.categories.indexOf(category);
  ...
  projectForm.categories.push(category);
}

const onSubmitUi = async () => {
